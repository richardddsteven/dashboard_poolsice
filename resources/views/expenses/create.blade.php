...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle" style="margin-bottom: 16px;">Input data pengeluaran baru.</p>
    </div>
</div>

<div class="card">
    <div class="card-header">
        <h3 class="card-title">Formulir Pengeluaran</h3>
    </div>
        ...
        <form action="{{ route('expenses.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 24px;">
                ...
                <div>
                    ...
                    <input type="date" id="date" name="date" class="form-control" value="{{ old('date', date('Y-m-d')) }}" required style="width: 100%;">
                    @error('date')
                ...
                <div>
                    ...
                    <div class="custom-select-wrapper" id="categorySelectWrapper">
                        ...
                        <div class="custom-options">
                            <div class="custom-option {{ (old('expense_category_id') == '') ? 'selected' : '' }}" data-value="" onclick="selectCategoryOption(this)">Pilih Kategori</div>
                            @foreach($categories as $category)
                                @php
                                    $isSelected = old('expense_category_id') == $category->id;
                                @endphp
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 24px;">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" placeholder="Contoh: Beli bensin mobil box" required style="width: 100%;">
                    @error('name')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="amount" name="amount" class="form-control" value="{{ old('amount') }}" placeholder="Contoh: 150000" min="0" required style="width: 100%;">
                    @error('amount')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea id="notes" name="notes" class="form-control" rows="4" placeholder="Tambahkan catatan jika perlu" style="width: 100%; resize: vertical;">{{ old('notes') }}</textarea>
                @error('notes')
                    <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                @enderror
            </div>
