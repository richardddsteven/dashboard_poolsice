...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Perbarui data pengeluaran</p>
    </div>
    ...
    <form method="POST" action="{{ route('expenses.update', $expense) }}">
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            <div>
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $expense->name) }}" required>
            </div>
            <div>
                ...
                <div class="custom-select-wrapper" id="categorySelectWrapper">
                    <div class="custom-select-trigger" onclick="document.getElementById('categorySelectWrapper').classList.toggle('open')">
                        <span id="categorySelectText">{{ $expense->category->name ?? 'Pilih kategori' }}</span>
                    ...
                    <div class="custom-options">
                        @foreach($categories as $category)
                            <div class="custom-option {{ old('expense_category_id', $expense->expense_category_id) == $category->id ? 'selected' : '' }}" data-value="{{ $category->id }}" onclick="selectCategory(this)">{{ $category->name }}</div>
                        @endforeach
                    </div>
            ...
            <div>
                ...
                <input type="number" id="amount" name="amount" class="form-control" value="{{ old('amount', $expense->amount) }}" min="0" required>
            </div>
            <div>
                ...
                <input type="date" id="date" name="date" class="form-control" value="{{ old('date', $expense->date instanceof \Carbon\Carbon ? $expense->date->format('Y-m-d') : $expense->date) }}" required>
            </div>
            <div>
                <label for="notes" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px; color: var(--text-main);">Catatan</label>
                <textarea id="notes" name="notes" class="form-control" rows="3">{{ old('notes', $expense->notes) }}</textarea>
            </div>
...
function selectCategory(el) {
    ...
    document.getElementById('categorySelectText').textContent = el.textContent.trim();
