...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle" style="margin-bottom: 16px;">Pendaftaran data supir baru.</p>
    </div>
</div>
        ...
        <form action="{{ route('drivers.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 24px;">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" placeholder="Masukkan nama supir" required style="width: 100%;">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="phone" name="phone" class="form-control" value="{{ old('phone') }}" placeholder="Contoh: 081234567890" required style="width: 100%;">
                    @error('phone')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 24px;">
                ...
                <div>
                    ...
                    <div class="custom-select-wrapper" id="zoneSelectWrapper">
                        ...
                        <div class="custom-options">
                            <div class="custom-option {{ (old('zone_id') == '') ? 'selected' : '' }}" data-value="" onclick="selectZoneOption(this)">Pilih Zona</div>
                            @foreach($zones as $zone)
                                @php
                                    $isSelected = old('zone_id') == $zone->id;
                                @endphp
                        ...
                        <input type="hidden" name="zone_id" id="zoneInput" value="{{ old('zone_id') }}" required>
                    </div>
                    @error('zone_id')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="username" name="username" class="form-control" value="{{ old('username') }}" placeholder="Contoh: supir.jimbaran" required style="width: 100%;">
                    @error('username')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-bottom: 24px;">
                <!-- Password Login -->
                <div>
                    <label for="password" style="display: block; margin-bottom: 8px; font-weight: 600; color: var(--text-main);">Password Login <span style="color: #ef4444;">*</span></label>
                    <input type="password" id="password" name="password" class="form-control" placeholder="Minimal 6 karakter" required style="width: 100%;">
                    @error('password')
                        <div style="color: #ef4444; font-size: 13px; margin-top: 6px;">{{ $message }}</div>
                    @enderror
                </div>
...
@push('scripts')
<script>
    // Initialize zone select value on load (for validation errors)
    document.addEventListener('DOMContentLoaded', function() {
        const oldZone = document.getElementById('zoneInput').value;
        if (oldZone) {
            const option = document.querySelector(`.custom-option[data-value="${oldZone}"]`);
            if (option) {
